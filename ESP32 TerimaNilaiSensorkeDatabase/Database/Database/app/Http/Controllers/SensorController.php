...
{
    // Ambil semua data masuk hari ini
    $masuk = Sensor::where('sensor_name', 'masuk')
                   ->whereDate('created_at', Carbon::today())
                   ->get();

    // Ambil semua linked_id dari data keluar
    $linkedKeluar = Sensor::where('sensor_name', 'keluar')
                          ->whereDate('created_at', Carbon::today())
                          ->pluck('linked_id')
                          ->filter(); // buang null

    // Filter: hanya tampilkan yang belum keluar
    $orangDiDalam = $masuk->whereNotIn('id', $linkedKeluar);

    return view('admin.sensors', [
        'masuk' => $orangDiDalam,
        'orangDiDalam' => $orangDiDalam->count()
    ]);
}

    // 🔘 Fungsi untuk ngeluarin orang berdasarkan ID
public function manualKeluar($id)
{
    $sensor = Sensor::find($id);

    if (!$sensor || $sensor->sensor_name !== 'masuk') {
        return response()->json(['error' => 'ID tidak valid atau bukan data masuk'], 400);
    }

    // Tambahkan record keluar dan simpan ID orang yang dikeluarkan
    $keluar = Sensor::create([
        'sensor_name' => 'keluar',
        'value' => 1,
        'linked_id' => $id, // 🔗 tandai siapa yang keluar
    ]);

    return response()->json([
        'message' => "Orang dengan ID $id berhasil dikeluarkan",
        'data' => $keluar
    ]);
}
